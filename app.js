...
function renderGroups() {
  ...
  const leftHtml = left.map(g => {
    const teams = RESULTS.groups[g] || [];
    const t = teams[0] || '';
    const d = TEAM_DATA[t];
    return `<div class="group-item">${d ? getFlag(d.code) : ''}<span class="group-code">${g}</span></div>`;
  }).join('');

  const rightHtml = right.map(g => {
    const teams = RESULTS.groups[g] || [];
    const t = teams[0] || '';
    const d = TEAM_DATA[t];
    return `<div class="group-item"><span class="group-code">${g}</span>${d ? getFlag(d.code) : ''}</div>`;
  }).join('');
...
function renderRounds() {
  ...
  r32.forEach(m => {
    ...
    html32 += `<div class="match">
      <div class="match-row"><span class="team-info">${d1 ? getFlag(d1.code) : ''}</span>
      <input type="number" class="score-input" min="0" placeholder="0" value="${h}"
        oninput="handleScore(${m.match}, 'home', this.value)"></div>
      <div class="match-vs">vs</div>
      <div class="match-row"><input type="number" class="score-input" min="0" placeholder="0" value="${a}"
        oninput="handleScore(${m.match}, 'away', this.value)">
      <span class="team-info">${d2 ? getFlag(d2.code) : ''}</span></div>
    </div>`;
  });
  document.getElementById('round16avos').innerHTML = html32;
}
