/* ============================================================
   FIFA 2026 Professional Bracket - app.js
   Renderizado visual del bracket con grupos
   ============================================================ */
...
function teamLabel(key) {
  ...
  if (!d) return key;
  return `<span class="fi fi-${d.code}"></span> ${d.name}`;
}

function teamFlag(key) {
  const d = TEAM_DATA[key];
  if (!d) return '🏴';
  return `<span class="fi fi-${d.code}" style="font-size: 1.8rem;"></span>`;
}
...
function renderGroups() {
  ...
  const leftContainer = document.getElementById('groupsLeft');
  const rightContainer = document.getElementById('groupsRight');

  if (!leftContainer || !rightContainer) return;

  leftContainer.innerHTML = leftGroups.map(g => {
    const teams = RESULTS.groups[g];
    return `
      <div class="group-card">
        <span class="group-flag">${teamFlag(teams[0])}</span>
        <span class="group-label">GRUPO ${g}</span>
      </div>
    `;
  }).join('');

  rightContainer.innerHTML = rightGroups.map(g => {
    const teams = RESULTS.groups[g];
    return `
      <div class="group-card">
        <span class="group-label">GRUPO ${g}</span>
...
function renderBracket() {
  const container = document.getElementById('bracketVisual');
  ...
  matchesData.forEach((m) => {
    ...
    html += `
      <div class="prediction-card">
        <div class="prediction-match-num">Partido ${m.match}</div>
        <div class="prediction-teams">
          <div class="prediction-team-row">
            <span class="prediction-team-name" title="${t1}">${t1}</span>
            <input type="number" class="prediction-input" min="0" placeholder="0" value="${h}"
              oninput="handleScoreChange(${m.match}, 'home', this.value)">
          </div>
          <div class="prediction-vs">vs</div>
          <div class="prediction-team-row">
            <input type="number" class="prediction-input" min="0" placeholder="0" value="${a}"
              oninput="handleScoreChange(${m.match}, 'away', this.value)">
            <span class="prediction-team-name" title="${t2}">${t2}</span>
          </div>
        </div>
      </div>
    `;
...
function submitPrediction(playerName) {
  showLoading(true, "Procesando pronóstico...");
...
async function loadLeaderboard() {
  ...
  try {
    ...
    const html = players.map((p, i) => `
      <div class="lb-row ${['lb-first', 'lb-second', 'lb-third'][i] || ''}">
    ...
    container.innerHTML = `
      <div class="leaderboard-table">${html}</div>
    `;
  } catch (e) {
